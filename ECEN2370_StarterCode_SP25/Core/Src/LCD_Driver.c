...
void LCD_Draw_Rectange_Fill(uint16_t Xpos, uint16_t Ypos, uint16_t len, uint16_t height, uint16_t color) {
	for(uint16_t i = Ypos; i < Ypos + height; i++) {
		LCD_Draw_Horizontal_Line(Xpos, i, len, color);
	}
}
...
void LCD_WriteString(uint16_t xPos, uint16_t Ypos, char* word) {
	for(int i = 0; i < strlen(word); i++) {
		...
		LCD_DisplayChar(xPos, Ypos, word[i]);
	}
	return;
}
